...
|         |replaces **crlf** sequences with **lf**'s.  - if **cr** is|
|         |the only line separator (like in hp-unix) it is replaced|
|         |with **lf**.  - **^Z**, **^Q** and **null** characters are removed,|
|         |- any BOM mark is removed.  - character set is converted|
|         |to utf-8.|
